...
AFTestState afTestStateReducer(AFTestState state, dynamic action) {
  if(action is AFStartPrototypeScreenTestContextAction) {
    return state.startTest(action.context, action.navigate, action.stateViews, action.stateViewId, action.routeParamId);
    ...
    return state.navigateToTest(action.test, action.navigate, action.stateView, action.stateViewId, action.routeParamId);
